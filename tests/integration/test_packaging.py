...
def test_sdist_wheel_and_clean_installation(tmp_path):
    ...
    invoke([sys.executable, "-m", "build", "--no-isolation", "--outdir", str(output)], ROOT)
    ...
    invoke([str(executable), "-m", "pip", "--isolated", "install", "--no-index", "--no-deps", str(wheels[0])], tmp_path)
    expected = f"1337 {metadata['version']}"
    assert invoke([str(executable), "-I", "-m", "fuzzy1337", "--version"], tmp_path).strip() == expected
    suffix = ".exe" if sys.platform == "win32" else ""
    assert invoke([str(scripts / f"1337{suffix}"), "--version"], tmp_path).strip() == expected
    assert "1337 repository quality gates" in invoke([str(scripts / f"1337-dev{suffix}"), "--help"], tmp_path)
    location = invoke([str(executable), "-I", "-c", "import fuzzy1337,json; print(json.dumps(fuzzy1337.__file__))"], tmp_path)
